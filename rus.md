...
Из всего что нам доступно на сегодняшний день, приём «машинка с клоунами»<a href="#note-1" class="reference">1</a>
...
К сожалению, до этого времени решение для обычных (не фоновых) изображений пока
...
Когда речь идёт об изображениях в теле страницы (которые являются частью
...
То что браузеры поддерживают SVG *и* SVG поддерживает медиазапросы и растровые
...
Хотя, с точки зрения отзывчивого дизайна, SVG работает безупречно, несколько
проблем с ним все же есть. К сожалению, `display: none` для `<image>` в SVG, так же
...
понятна. Для приёма «машинка с клоунами» используются те же медиазапросы, что и
при разработке обычных адаптивных сайтов.
...
Проблемы начинаются когда мы добавляем SVG в верстку резинового макета. Когда в первый раз
...
Если добавить SVG в тэг `<img>` с относительно заданной шириной, например 70% от ширины области
просмотра, при увеличении или уменьшении контейнера вследствие изменения размера
окна или изменений в CSS, изображение будет изменяться в соответствии с новыми
обстоятельствами.
...
Хоть ранее мы и убедились что браузеры понимают SVG если он добавлен в документ
...
может загрузить внешние растровые изображения в WebKit и FireFox. Я обратилась
в службу поддержки Chrome с просьбой снять запрет на импорт растровых
изображений в SVG.
...
… блокируется в некоторых браузерах и следовательно такое решение неприемлемо.
...
Все браузеры поддерживают тэг `<object>`. Тэг `<img>` не будет работать так,
как нам нужно, пока политика безопасности, которая соблюдается браузерами, не
будет пересмотрена. Вместо него мы можем использовать тэг `<object>`.
...
Его использование не делает код сложнее:
...
Благодаря `viewbox` и `preserveAspectRatio`, объявленным в SVG-файле, `<object>`
по умолчанию сохраняет указанное соотношение сторон. Такое поведение можно
изменить с помощью атрибутов HTML или CSS.
...
разметку с помощью тэга `<svg>`][20]. Преимуществом такого решения является
то, что не происходит http-запрос, нужный для его получения.

К сожалению, Chrome и Safari интерпретируют SVG как обычный блочный элемент
и не поддерживают атрибут `preserveAspectRatio` при таком подключении
(хотя они поддерживают `preserveAspectRatio` когда SVG подключён через тэг `<object>`).

Ещё один недостаток — это что такое решение не позволяет добавить
...
придется использовать `background-image` с `height` и `width` для `<svg>`.

### Деградация в IE

Элемент `<object>` поддерживают все браузеры, даже мобильные. Но этот приём
работает только если браузер поддерживает SVG. Таким образом, это решение
не будет работать в IE 8 или Android 2.3 и более старых их версиях. Для старых
браузеров мы реализуем деградацию к тегу `img`. Нам придется сделать два запроса:
SVG-файла и растрового изображения, которое мы хотим отобразить.
...
Чтобы этого избежать, можно использовать условные комментарии IE:
...
показаться странным, но легко объясняется тем что в данном случае Microsoft
пытался следовать спецификации буквально. Спецификация говорит что data
...
Откройте [нашу первую страницу][21] и [вторую страницу][22], затем зайдите в
...
Прелесть этого приёма состоит в том, что изображение изменяется когда изменяется
размер его контейнера. Можете установить для изображения с
...
## Остальные достоинства этого приёма
...
Не нужно засорять HTML дополнительным неиспользуемым кодом чтобы получить
...
чтобы CSS был полностью загружен до начала загрузки изображений. У меня
...
### Android 2.3 и более старые его версии

Android 2.3 и более старые не поддерживает SVG. Я нашла три возможных решения,
которые еще нужно довести до ума.

#### `<SVG>` с `background-image`

Вместо `<object>` можно использовать встроенный `<svg>`. Так как IE 8,
Android 2.3 и более старые их версии не поддерживают SVG, с помощью CSS эти
браузеры могут выделить под `<svg>` пространство с `height` и `width`, а затем объявить
...
Такой код отобразит меньший PNG на телефонах с Android. К сожалению, для всех
...
`aria-labeled-by`
* Добавление `tab-index="0"` чтобы `<object>` получил фокус без смены
порядка табуляции
...
Если отсутствие возможности сохранения после правого клика мышкой — это для вас
...
<a href="#note-1" id="note-1" class="reference">1</a> Машинка с клоунами *(англ.
